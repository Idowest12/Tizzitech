...
code = code.replace(
  /<button\s+type="submit"\s+className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3\.5 px-6 rounded-xl text-sm tracking-widest uppercase transition-colors flex items-center justify-center gap-2 shadow-lg shadow-blue-500\/20"\s*>\s*Authenticate Portal\s*<\/button>/,
  `<button 
              type="button"
              onClick={handleLogin}
