const fs = require('fs');
let code = fs.readFileSync('src/AdminApp.tsx', 'utf-8');

code = code.replace(
  /  const handleLogin = async \(e: React\.FormEvent\) => \{\n    e\.preventDefault\(\);\n    setError\(''\);\n    \n    try \{\n      const provider = new GoogleAuthProvider\(\);\n      await signInWithPopup\(auth, provider\);\n    \} catch \(err: any\) \{\n      setError\(err\.message \|\| 'Authentication failed\.'\);\n    \}\n  \};/,
  `  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    
    if (!adminEmail) {
       setError('Please enter your admin email address.');
       return;
    }
    
    setLoading(true);
    try {
      const res = await fetch('/api/admin/send-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: adminEmail })
      });
      const data = await res.json();
      
      if (data.devOtp) {
        console.log("DEV OTP:", data.devOtp); // For dev preview
        alert("SMTP is not configured! Simulated Email OTP: " + data.devOtp + "\\n\\nPlease configure SMTP_HOST, SMTP_USER, SMTP_PASS in the Environment settings for real emails to work.");
      }
      
      setAuthStep('otp');
    } catch (err: any) {
      setError(err.message || 'Failed to request access code.');
    } finally {
      setLoading(false);
    }
  };`
);

code = code.replace(
  /              <div>\n                <p className="block text-\[10px\] font-bold text-neutral-500 uppercase tracking-widest mb-2 text-center">\n                  SIGN IN WITH GOOGLE\n                <\/p>\n              <\/div>\n              \{error && \([\s\S]*?\)\]\n              \)\}\n              <button \n                type="button"\n                onClick=\{handleLogin\}\n                className="w-full bg-neutral-100 hover:bg-white text-black font-bold py-3\.5 px-6 rounded-xl text-sm tracking-widest uppercase transition-colors flex items-center justify-center gap-3 shadow-lg shadow-white\/5"\n              >\n                <svg className="w-5 h-5" viewBox="0 0 24 24">\n                  <path fill="#4285F4" d="M22\.56 12\.25c0-\.78-\.07-1\.53-\.2-2\.25H12v4\.26h5\.92c-\.26 1\.37-1\.04 2\.53-2\.21 3\.31v2\.77h3\.57c2\.08-1\.92 3\.28-4\.74 3\.28-8\.09z"\/>\n                  <path fill="#34A853" d="M12 23c2\.97 0 5\.46-\.98 7\.28-2\.66l-3\.57-2\.77c-\.98\.66-2\.23 1\.06-3\.71 1\.06-2\.86 0-5\.29-1\.93-6\.16-4\.53H2\.18v2\.84C3\.99 20\.53 7\.7 23 12 23z"\/>\n                  <path fill="#FBBC05" d="M5\.84 14\.09c-\.22-\.66-\.35-1\.36-\.35-2\.09s\.13-1\.43\.35-2\.09V7\.07H2\.18C1\.43 8\.55 1 10\.22 1 12s\.43 3\.45 1\.18 4\.93l2\.85-2\.22\.81-\.62z"\/>\n                  <path fill="#EA4335" d="M12 5\.38c1\.62 0 3\.06\.56 4\.21 1\.64l3\.15-3\.15C17\.45 2\.09 14\.97 1 12 1 7\.7 1 3\.99 3\.47 2\.18 7\.07l3\.66 2\.84c\.87-2\.6 3\.3-4\.53 6\.16-4\.53z"\/>\n                <\/svg>\n                Sign In with Google\n              <\/button>/,
  `              <div>
                <label className="block text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2 text-center">
                  ADMIN EMAIL
                </label>
                <input
                  type="email"
                  value={adminEmail}
                  onChange={e => setAdminEmail(e.target.value)}
                  placeholder="admin@example.com"
                  className="w-full bg-black border border-neutral-800 rounded-xl px-4 py-3 text-center text-white focus:outline-none focus:border-blue-500 transition-colors"
                  required
                />
              </div>
              {error && (
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex gap-3 items-start animate-shake">
                  <ShieldAlert className="h-5 w-5 text-red-500 flex-shrink-0 mt-0.5" />
                  <p className="text-xs text-red-400 font-medium leading-relaxed">{error}</p>
                </div>
              )}
              <button 
                type="submit"
                disabled={loading || !adminEmail}
                className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-bold py-3.5 px-6 rounded-xl text-sm tracking-widest uppercase transition-colors flex items-center justify-center gap-3 shadow-lg shadow-blue-500/10"
              >
                {loading ? 'Requesting...' : 'Request Access Code'}
              </button>`
);

fs.writeFileSync('src/AdminApp.tsx', code);
